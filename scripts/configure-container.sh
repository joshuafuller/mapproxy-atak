...
if [[ -z "$host" || -z "$contact_url" ]]; then
  echo "Usage: ./scripts/configure.sh <LAN-host-or-IP> <contact-url> [port]" >&2
  echo "Example: ./scripts/configure.sh 192.168.1.50 https://example.com/contact 8080" >&2
...
printf 'Start with: docker compose up -d --force-recreate --wait\n'
